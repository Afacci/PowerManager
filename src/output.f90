...
!        rbuffer(4) = windEl(i)
  ...
  if(capacityES.gt.zero) then
     u = u + 1
     call prepareFile(u,'ElectricalStorageSOC',path)
     write(u,*) '#--------------------------------------------------------------------------#'
     write(u,*) '#  file ElectricalStorageSOC.dat; contains the state of charge of the electrical storage.  '
     write(u,*) '#--------------------------------------------------------------------------#'
     write(u,*)
     buffer20(1) = 'Time [h]            '
     buffer20(2) = 'SOC in [%]          '
     ...
     do i=1,nTime
        rbuffer(1) = t(i)
        kk = setPoint(i,:)
        rbuffer(2) = 100*level/capacityES
        rbuffer(4) = level
        level = elStorageLevelUpdate(level,kk,i)
        rbuffer(3) = 100*level/capacityES
        rbuffer(5) = level
        write(u,'(3(F5.2,15X),2(ES11.2E2,9X))') (rbuffer(j), j=1,5)
!        kk(4) = nSp(4)
!        kk(3) = 1
!        kk(1) = nSp(1)
!        print*, 'debug',  constraints(kk,i), elSelfCons(kk,i), elProd(kk,1)
     enddo
  endif
